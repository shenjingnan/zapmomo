...
mod tests {
    ...
    fn test_registry_parses() {
        ...
        assert_eq!(
            models.len(),
            23,
            "应为 7 个首批（含 2 KWS）+ 5 个 ASR + 2 个新 TTS + 3 个新 ASR + 2 个流式 Paraformer + 1 个新 KWS（gigaspeech）+ 2 个 Kokoro TTS + 1 个 Qwen3-ASR + 1 个 audiocpp PocketTTS + 1 个 audiocpp OmniVoice + 1 个 audiocpp VoxCPM2（LLM 条目已随本地推理移除）"
        );
        assert!(
            models
                .iter()
