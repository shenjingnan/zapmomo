...
2. **外放（扬声器）为主要场景**：回声过滤必须默认开启且偏保守——宁可偶尔打断不灵，
   不能自我打断。
3. **打断后 reset ASR 流**（2026-08-29 验收修订；原设计为不 reset 保话头）：流式识别
   是流内累积语义，保留流会把打断前喂入的播报回声与用户语音叠加进最终转写（实测
   多次说话内容越叠越多）；重置后转写只含「打断之后」说的话，打断瞬间已出口的
   半截话随流丢弃，回声漏网仍由后置文本比对兜底。
