...
压制带 **5s 耐心窗口**（`AWAITING_REPLY_PATIENCE_MS`，时间戳惰性判定，与插播新鲜期同
款模式）：用户句落屏时打点，窗口内插播暂存不抢屏；回复始终未始（如 LLM 出错）超过窗
口后，插播恢复正常展示（最新发言胜出）——等回复的耐心是有限的，插播不能被一个永不来
的回复无限期压制。
（实施期勘误：初版规格为无界压制，与「用户句静置期间插播正常补展示」的测试预期矛盾，
TDD 红灯阶段暴露后修订为本语义。）
