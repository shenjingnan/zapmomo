...
interface KwsModelDialogProps {
  open: boolean;
  onClose: () => void;
}

/**
 * 选择唤醒词模型弹窗（与 LLM 选择模型弹窗同款交互）：
 * 内置预设（未安装→下载；已安装→设为当前 / 卸载；当前→标记）。
 * 监听中切换由 useKwsModelSwitch 自动重启监听；语音会话运行中则下次会话生效。
 * 卸载确认框嵌套在此弹窗内。
 */
export function KwsModelDialog({ open, onClose }: KwsModelDialogProps) {
  const { voice } = useRuntime();
  const switcher = useKwsModelSwitch();
  const [confirmModel, setConfirmModel] = useState<LibraryModel | null>(null);
  /** 正在设为当前的预设 id；期间按钮转圈并禁用（监听中切换含重启监听耗时） */
