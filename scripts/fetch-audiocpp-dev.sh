...
#
# 编译参数与 release.yml 一致：裁剪 pocket_tts+omnivoice+voxcpm2+qwen3_tts+qwen3_asr
# + DEPLOYMENT_BUILD
...
if [ "${1:-}" = "--build" ]; then
  ...
  cmake -S .audiocpp-src -B .audiocpp-build \
    -DAUDIOCPP_MODEL_SET=custom -DAUDIOCPP_MODELS=pocket_tts,omnivoice,voxcpm2,qwen3_tts,qwen3_asr \
