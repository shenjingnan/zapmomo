...
//! 手动运行（Anthropic）：
//! ```bash
//! ANTHROPIC_API_KEY=<your-key> cargo test --test remote_llm_smoke -- --ignored --nocapture
//! ```
use std::sync::Arc;
use std::sync::atomic::AtomicBool;

use zapmomo::llm::anthropic::AnthropicProvider;
...
#[test]
#[ignore = "真实 API 冒烟：需 ANTHROPIC_API_KEY 环境变量"]
fn test_anthropic_streaming_smoke() {
    let api_key = std::env::var("ANTHROPIC_API_KEY").expect("需设置 ANTHROPIC_API_KEY");
    let config = ResolvedLlmConfig {
        enabled: true,
        cli_tools: false,
        sprite_tool: true,
    ...
    let mut provider = AnthropicProvider::new(&config).unwrap();
